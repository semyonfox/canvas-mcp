...
export const announcementTools: ToolDef[] = [
    {
        ...
        handler: async (args, { canvas }) => {
            const announcements = await canvas.collectPaginated("/api/v1/announcements", {
                per_page: 100,
    ...
    },
    {
        ...
        handler: async (_args, { canvas }) => {
            const notifications = await canvas.get("/api/v1/accounts/self/account_notifications", {});
        ...
        handler: async (args, { canvas }) => {
            const announcement = await canvas.post(
                `/api/v1/courses/${args.course_id}/discussion_topics`,
                {
                    ...
                    ...(args.delayed_post_at ? { delayed_post_at: args.delayed_post_at } : {}),
                },
            );
            return jsonResult(announcement);
        },
    },
    {
        ...
        description: "Delete an announcement from a course. Requires educator permissions.",
        inputSchema: z.object({
            course_id: canvasId,
            announcement_id: canvasId,
        }),
    ...
    },
    {
        ...
        description:
            "Delete multiple announcements from a course serially to respect Canvas rate limits. Requires educator permissions.",
        inputSchema: z.object({
            course_id: canvasId,
            announcement_ids: z.array(canvasId),
        }),
        handler: async (args, { canvas }) => {
            const results = [];
            for (const id of args.announcement_ids) {
                results.push(
                    await canvas.delete(
